...
// `n/**`n * Force video to 9:16 aspect ratio (1080x1920)`n * Uses a combination of cropping and padding to ensure the ratio is exact`n */`nasync function forceVerticalRatio(inputPath, outputPath) {`n  return new Promise((resolve, reject) => {`n    ffmpeg(inputPath)`n      .complexFilter([`n        "scale=iw*min(1080/iw,1920/ih):ih*min(1080/iw,1920/ih)",`n        "pad=1080:1920:(ow-iw)/2:(oh-ih)/2:black"`\n      ])`n      .outputOptions("-c:v libx264", "-crf 23", "-preset medium")`n      .on("end", () => resolve(outputPath))`n      .on("error", (err) => reject(err))`n      .save(outputPath);`n  });`n}`n`n/**`n * Apply visual uniqueness filters to bypass "Reused Content" flags`n * Includes subtle color shift, slight zoom, and mirroring`n */`nasync function applyVisualUniqueness(inputPath, outputPath) {`n  return new Promise((resolve, reject) => {`n    ffmpeg(inputPath)`n      .videoFilters([`n        "eq=saturation=1.1:contrast=1.05",`n        "crop=iw*0.95:ih*0.95",`n        "hflip"`\n      ])`n      .outputOptions("-c:v libx264", "-crf 23", "-preset medium")`n      .on("end", () => resolve(outputPath))`n      .on("error", (err) => reject(err))`n      .save(outputPath);`n  });`n} `n`nmodule.exports = {`n  getVideoDuration,`n  getVideoDimensions,`n  getAllFillerVideos,`n  getRandomPosition,`n  createFillerChain,`n  createSplitScreen,`n  forceVerticalRatio,`n  applyVisualUniqueness`n};
